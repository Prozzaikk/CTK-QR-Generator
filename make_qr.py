...
def make_qr(data_to_qr, filename, path_to_save, qr_type=".png", background_color="white", foreground_color="dark"):
    ...
    if len(data_to_qr) == 0 or len(filename) == 0 or len(path_to_save) == 0 or data_to_qr is None or filename is None or path_to_save is None:
        ...
        if "-" in data_to_qr:
            ...
            for i in range(int(data_seq[0]), int(data_seq[1]) + 1):
                ...
                qrcode.save(qr_url, dark=foreground_color, light=background_color, border=2, scale=5)
            ...
            qr_code.save(qr_url, dark=foreground_color, light=background_color, border=2, scale=5)
